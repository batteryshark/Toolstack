...
class JsonApi(unittest.TestCase):
    ...
    def test_policy_put_refuses_to_flatten_path_scoped_rules(self):
        self._create("hermes")
        # a path-aware client (or brokerctl) sets a path-scoped rule
        self.client.put("/api/callers/hermes/policy", headers=self._auth(),
                        json={"allow": ["kv.GET /items/**"]})
        # a coarse (verb-level) PUT omitting it is refused (409), not silently flattened
